...
function isTriangle(a, b, c) {
  const [largeSide, smallSide1, smallSide2] = [a, b, c].sort((x, y) => y - x);
  return (smallSide1 + smallSide2) > largeSide;
  // throw new Error('Not implemented');
}
...
function isInsideCircle(circle, point) {
  const { radius, center: { x, y } } = circle;
...
function findFirstSingleChar(str) {
  function recurFind(s) {
    if (s.length < 2 || !(s.includes(s[0], 1))) {
      return s && s[0] ? s[0] : null;
    }
    const newStr = s.replace(new RegExp(`${s[0]}`, 'g'), '');
    if (newStr.length) {
      return recurFind(newStr);
    }
    return null;
  }
  return recurFind(str);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const { min, max } = Math;
  return `${isStartIncluded ? '[' : '('}${min(a, b)}, ${max(a, b)}${isEndIncluded ? ']' : ')'}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < Str.length; i += 1) {
    ...
    if (bracketToclose) {
      ...
      if (Str[i] && Str[i] !== toClose[toClose.length - 1]) {
        return false;
      }
      if (Str[i] === toClose[toClose.length - 1]) {
        toClose.pop();
      }
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  return matrix.map((x) => x.filter((y) => y));
  // throw new Error('Not implemented');
}
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    ...
    if ([position[0][i], position[1][i], position[2][i]].filter((x) => x === '0').length === 3) {
      return '0';
    }
    if ([position[i][0], position[i][1], position[i][2]].filter((x) => x === '0').length === 3) {
      return '0';
    }
    if ([position[i][0], position[i][1], position[i][2]].filter((x) => x === 'X').length === 3) {
      return 'X';
    }
  }
  if ([position[0][0], position[1][1], position[2][2]].filter((x) => x === 'X').length === 3) {
    return 'X';
  }
  if ([position[0][0], position[1][1], position[2][2]].filter((x) => x === '0').length === 3) {
    return '0';
  }
  if ([position[0][2], position[1][1], position[2][0]].filter((x) => x === '0').length === 3) {
    return '0';
  }
  if ([position[0][2], position[1][1], position[2][0]].filter((x) => x === 'X').length === 3) {
    return 'X';
  }
  return undefined;
}
